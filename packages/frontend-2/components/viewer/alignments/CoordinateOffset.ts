...
export function cadToSpeckle(
  ...
  return {
    position: transformCadToSpecklePoint(pos, offset),
    target: transformCadToSpecklePoint(target, offset)
  }
}
...
export function speckleToCad(
  ...
  return {
    position: transformSpeckleToCadPoint(pos, offset),
    target: transformSpeckleToCadPoint(target, offset)
  }
}
...
export function calcTransformFromCalibrationPoints(
  ...
  const speckleDistance = distance2D(
    { x: speckleA.x, y: speckleA.y },
    { x: speckleB.x, y: speckleB.y }
  )
  const scale =
